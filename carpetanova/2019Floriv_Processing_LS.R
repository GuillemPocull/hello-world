#script to process 2019 florivory interaction data
...
# The damage on the face, the top OR the chin of the flower is counted as 1 and if not as 0.  
raw$floriv_damage <- ifelse((raw$face=="1") | (raw$top=="1") | (raw$chin=="1"), 1, 0)
...
format_int <- raw %>% dplyr::rename(plant.id.field = plant_ID, presence = floriv_damage, date = sampling_date, inflor.tot.fl = focal_flowers, inflor.tot.fr = focal_fruit, flower.pos = flower_ID)
...
format_int <- format_int %>% add_column(dataset.id = "floriv_2019", interaction = "floriv", year = "2019", survey.id = NA, inflor.id = NA, count = NA)
...
write.csv(format_int, "Processing/2019Floriv_Interaction_Formatted.csv", row.names = FALSE)
