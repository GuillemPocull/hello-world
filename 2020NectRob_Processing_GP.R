...
raw <- read_csv("Raw/2020NectRob_RAW_GP.csv", col_types = cols(hole_robbing = col_character()))
str(raw)
...
format_int <- binary_dat %>% dplyr::rename(plant.id.field = ID, presence = hole_robbing)
...
format_int <- format_int %>% add_column(dataset.id = "nect.rob_2020", interaction = "nect.rob", year = "2020", date = NA, survey.id = NA, inflor.id = NA, inflor.tot.fl = NA, inflor.tot.fr = NA, flower.pos = NA, count = NA)
...
write.csv(format_int, "Processing/2020NectRob_Interaction_Formatted.csv", row.names = FALSE)
